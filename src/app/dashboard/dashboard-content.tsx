...
import { startToday } from "@/lib/actions/boards";
...
export default function DashboardContent({
  ...
  async function handleStartToday(boardId: string) {
    ...
    await startToday(boardId);
    // Optimistically update UI: move top 3 not_started to in_progress
    setBoards((prev) =>
      prev.map((board) => {
        if (board.id !== boardId) return board;
        const notStarted = board.in_progress_cards.length === 0
          ? [] // will be populated by revalidation
          : board.in_progress_cards;
        return board;
      })
    );
      ...
      <>
        ...
        <div className="flex flex-col items-center justify-center px-6 py-20 text-center">
          ...
          <Button
            asChild
            size="lg"
            className="gap-2 bg-gradient-to-r from-blue to-purple hover:from-blue/90 hover:to-purple/90 shadow-lg"
          >
          ...
          <Card className="bg-gradient-to-br from-blue/15 via-blue/8 to-background border-blue/30 shadow-lg shadow-blue/5">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="secondary"
                  className="text-xs bg-blue/10 text-blue-700 border-blue/20"
                >
                  G{activeBoard.grade}
                </Badge>
              </div>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="flex gap-2">
                <Button
                  asChild
                  size="sm"
                  className="flex-1 gap-1 bg-blue hover:bg-blue/90"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
              {boards.slice(0, 4).map((board, index) => {
                ...
                return (
                  ...
                  >
                    <Card
                      className={`hover:bg-${colorClass}/5 transition-all duration-200 cursor-pointer bg-gradient-to-br from-${colorClass}/10 via-${colorClass}/5 to-background border-${colorClass}/25 hover:border-${colorClass}/40 hover:shadow-lg hover:shadow-${colorClass}/10`}
                    >
                      <CardContent className="p-3">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span
                            className={`text-xs text-${colorClass}-600 dark:text-${colorClass}-400 ml-2 font-medium`}
                          >
                            {p}%
                          </span>
                        </div>
